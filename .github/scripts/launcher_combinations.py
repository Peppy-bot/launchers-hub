...
# The states `peppy stack list` reports for an instance the stack is still
# running. It reports `finished` and `failed` for the rest.
LIVE_INSTANCE_STATES = frozenset({"starting", "running"})
...
def instance_configurations(plan):
    """Every node instance a flattened launcher deploys, as the canonical
    text of its source and its whole configuration."""
    return {
        canonical([deployment.get("source"), instance])
        for deployment in plan.get("deployments", [])
...
def running_states(candidate, resolutions):
    ...
    return [instance_configurations(launched.plan), instance_configurations(plan)]
...
def running_copy_instances(listing, name):
    ...
    A copy keeps an instance on its record after that instance stops, so what
    a launch proves is read off the stack's own report of each instance's
    state: a robot the simulation would not stand leaves its initializer
    finished and the rest of the copy up."""
    stack = json.loads(listing)
    running = {
        instance["instance_id"]
        for core_node in stack["core_nodes"]
        for node in core_node["stack"]["nodes"]
        for instance in node["instances"]
        if instance.get("state") in LIVE_INSTANCE_STATES
    }
    return sorted(
        instance_id
        for core_node in stack["core_nodes"]
...
def launch_start_to_end(launch, rebuild, run):
    ...
    deploys. The joined copy is held to the instances its preview gave it, so
    a join that comes up as another robot than the one planned fails the
    launch."""
