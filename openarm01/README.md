...
|---|---|
| `openarm01_teleop.json5` | the real robot |
| `openarm01_teleop_isaac.json5` | Isaac Sim |
| `openarm01_teleop_mujoco.json5` | MuJoCo |

## Before launching

The node repos must be registered with the daemon and the nodes built. The top-level README in `openarm01_nodes` walks through the whole sequence; in short, every repo gets a `peppy repo add` (followed by `peppy repo refresh`), and every node gets a `peppy node add <path> -sb`. Then verify:
...
```sh
peppy stack launch ./openarm01_teleop_mujoco.json5
```
...
The sim keeps loading after `Launch complete`, and Isaac can take a minute. Check instance health with `peppy stack list` and watch the sim's log with `peppy node info openarm01_robot_initializer_<engine>:v1`.
